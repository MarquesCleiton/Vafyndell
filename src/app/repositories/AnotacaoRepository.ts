...
export class AnotacaoRepository {
  ...
  static async updateAnotacao(anotacao: AnotacaoDomain): Promise<AnotacaoDomain> {
    ...
    const updated = await ScriptClient.controllerUpdateByIndex({
      tab: this.TAB,
      index: anotacao.index, // 👈 update sempre pelo index
      attrs: anotacao,
      folderId: this.FOLDER_ID,
    });
    ...
    const anotacaoFinal: AnotacaoDomain = {
      ...anotacao,
      ...updated,
      id: Number(updated?.id || anotacao.id), // 👈 preserva id da planilha
      index: updated?.index || anotacao.index,
    };
  ...
  static async getAllAnotacoes(): Promise<AnotacaoDomain[]> {
    ...
    return Array.isArray(onlineList)
      ? onlineList.map(a => ({
          ...a,
          id: Number(a.id), // 👈 garante id real
          index: a.index,
        }))
      : [];
  }
  ...
  static async forceFetchAnotacoes(): Promise<AnotacaoDomain[]> {
    ...
    const anotacoesComId = onlineList.map(a => ({
      ...a,
      id: Number(a.id), // 👈 preserva id real
      index: a.index,
    }));
  ...
  static async deleteAnotacao(id: number): Promise<boolean> {
    const db = await this.getDb();
    const anotacao = await db.get<AnotacaoDomain>(this.STORE, id);

    if (!anotacao) {
      console.warn('[AnotacaoRepository] Anotação não encontrada no cache:', id);
      return false;
    }

    await ScriptClient.controllerDeleteByIndex({
      tab: this.TAB,
      index: anotacao.index, // 👈 exclusão pelo index
    });
